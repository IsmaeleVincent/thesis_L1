...
def process_fit(k):
    ...
    for i in range(1,3):
        ...
        diff_eff_fit[2+i,:]=diff_eff[:,6+2*i].copy()
    def fit_func(x, bcr1, bcr2, bcr3, mu1,phi,phi1,d, lambda_par, sigma):
        ...
        #plt.plot(aaa)
        return aaa
    P0= [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] # fit_res[0]# [*fit_res[0,:-1],0.0005] #  [5,0,2.6e-3] # 
    if (fitting):
        B=([3, 0, 0, 2e-3, 0,0,70,500,0.0003],[15, 6,1, 3e-3, 2*pi,2*pi,86,1000,0.0005])        
...
if (plotting):
    for k in range(len(foldername)):
        ...
        for i in range(1,3):
            diff_eff_fit[2-i,:]=diff_eff[:,6-2*i].copy()
            diff_eff_fit[2+i,:]=diff_eff[:,6+2*i].copy()
        def plot_func(x, bcr1, bcr2, bcr3, mu1, phi,phi1,d,lambda_par, sigma):
            d=d/np.cos(tilt[k]*rad)
            sigma1= (sigma**2+1/lambda_par**2)**0.5
            wl=np.linspace(mu1-2.5*sigma, mu1+1/lambda_par+3.5*sigma1, 10000)
            a = rho(wl,lambda_par, mu1, sigma)/sum(rho(wl,lambda_par, mu1, sigma))
...
tot_res = np.zeros((len(foldername), 10))
for k in range(len(foldername)):
    print(foldername[k])
    data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
    fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_1line.mpa',skiprows=1)
...
title=["bcr1","bcr2","bcr3","mu","phi","phi1","thicknes", "lambda_par", "sigma"]
